...
import {
  ...
  MessageType,
  Side
} from '../../apps/node-ingestion/src/message-processor/index.js'
import {
  MessageProcessingError,
  ValidationError,
  BackpressureError
} from '../../apps/node-ingestion/src/errors/index.js'
...
describe('MessageProcessor', () => {
  ...
  beforeEach(() => {
    ...
    const mp = msgpack()
    validMessageBytes = mp.encode(validMessageData)
  ...
  describe('Message Processing', () => {
    ...
    test('should handle invalid message with validation error', async () => {
      const mp = msgpack()
      const invalidBytes = mp.encode(invalidMessageData)
  ...
  describe('Latency Measurement', () => {
    ...
    test('should measure latency with artificial delay', async () => {
      ...
      processor.msgpack.decode = jest.fn().mockImplementation((buffer) => {
        // Add 1ms delay
        const start = Date.now()
        while (Date.now() - start < 1) {
          // Busy wait for 1ms
        }
        return originalDecode.call(processor.msgpack, buffer)
      })
  ...
  describe('Statistics Tracking', () => {
    ...
    test('should track error statistics correctly', async () => {
      ...
      const invalidData = { ...validData, messageType: 'INVALID' }

      // Process valid message
      await processor.processMessage(mp.encode(validData))

      // Process invalid message (validation error)
      await processor.processMessage(mp.encode(invalidData))
  ...
  describe('Message Validation', () => {
    test('should fail validation for missing required fields', async () => {
      ...
      const result = await processor.processMessage(mp.encode(incompleteData))
      expect(result.success).toBe(false)
    ...
    test('should fail validation for invalid message type', async () => {
      ...
      const result = await processor.processMessage(mp.encode(invalidData))
      expect(result.success).toBe(false)
    ...
    test('should fail validation for invalid price and quantity', async () => {
      ...
      let result = await processor.processMessage(mp.encode(invalidPriceData))
      expect(result.success).toBe(false)
      ...
      result = await processor.processMessage(mp.encode(invalidQuantityData))
      expect(result.success).toBe(false)
